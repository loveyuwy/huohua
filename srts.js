...
!(async () => {
    ...
    // --- 智能解析逻辑 ---
    $.log(`[调试] 原始参数内容: ${JSON.stringify(rawArg)}`);
    ...
    for (let val of items) {
        ...
        if (!isNaN(s) && !s.includes('@')) {
            advanceDays = parseInt(s);
        } 
        // 2. 如果包含 @，认为是生日数据
        else if (s.includes('@')) {
            // 兼容可能的分号分隔
            birthdayList.push(...s.split(';').filter(x => x.trim().includes('@')));
        }
    }
    ...
    for (let i = 0; i <= advanceDays; i++) {
        ...
        for (let item of birthdayList) {
            ...
            // 修正特殊横杠
            let targetDate = parts[2].trim().replace(/[−—﹣－.／\\]/g, '-');
            
            // 补全 MM-DD 格式 (例如把 3-6 变成 03-06)
            if (targetDate.includes('-')) {
                targetDate = targetDate.split('-').map(v => v.padStart(2, '0')).join('-');
            }
