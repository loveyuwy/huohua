...
class DJG {
  ...
  init(widgetFamily = config.widgetFamily) {
    ...
    this.scale = (() => {
      let {height} = this.getWidgetSize('small');
      return parseFloat((height/155).toFixed(2));
    })();
  ...
  isUpdate(cacheKey, useCache = true, time = parseInt(this.settings.refreshAfterDate)) {
    ...
    if(lastTime != lastTime || lastTime < (nowTime - 1000*60*time) || lastTime == nowTime) {
  ...
  async httpGet (url, json = true, useCache = true, options = null, method = 'GET') {
    let str = url + options?.headers + options?.body;
    ...
    if (this.isUpdate(cacheKey, useCache) || !Keychain.contains(cacheKey)){
      try {
        ...
        req.method = method;
        if(options){
          const {headers, body, from} = options;
          !!headers && JSON.stringify(headers) != "{}" && (req.headers = headers);
          if(!!body) {
          ...
          if(!!from && JSON.stringify(from) != "{}"){
            Object.keys(from).forEach((key) => {
              req.addParameterToMultipart(key, from[key])              
            })
          }
        }
  ...
  getWidgetSize(widgetFamily) {
    ...
    if (!phoneSize) {
      phoneSize = this.phoneSizes()['2556'];
      screenScale = 3;
    }
      ...
      },
      {
        ...
        onClick: async () => {
          const actions = [
              ...
              val: "白天 | 夜间",
              onClick: async () => {
                let index = await this.setCustomAction("背景颜色", "⒈白天 ⒉晚上\n请自行去网站上搜寻颜色（Hex 颜色）", {
                  lightBgColor: '浅色模式',
                  darkBgColor: '深色模式',
                });
                if(index) {
  ...
  async getLocation(locTime = 20) {
    ...
    if (!Keychain.contains(cacheKey) || 
      (!!this.djgSettings.button[2] && this.isUpdate('locUPtime', "位置获取", locTime))) {
