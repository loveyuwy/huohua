/*
 * iKuuu 二合一脚本 (自动获取Cookie + 自动签到)
 * 兼容：Surge, Loon, Quantumult X
 * * 逻辑说明：
 * 1. 当通过浏览器访问页面触发重写时 -> 执行 Cookie 获取
 * 2. 当通过定时任务或手动点击运行时 -> 执行 签到
 */
...
function GetCookie() {
    if ($request && $request.headers) {
        ...
        if (cookie) {
            ...
            if (saved) {
                $.msg($.name, "Cookie 获取成功 🎉", "签到功能已就绪");
                console.log("✅ Cookie 已更新: " + cookie);
            } else {
                $.msg($.name, "获取失败", "无法写入数据");
            }
        }
    }
    $.done();
...
function Checkin() {
    ...
    if (!cookie) {
        $.msg($.name, "签到失败", "🚫 未找到 Cookie，请先开启重写并访问网页获取");
        $.log("❌ 本地无 Cookie 数据");
        $.done();
        return;
    }
    ...
    $.post(myRequest, (error, response, data) => {
        if (error) {
            $.msg($.name, "签到失败", "网络连接错误");
            $.log("❌ 网络错误: " + error);
        } else {
            try {
                const obj = JSON.parse(data);
                if (obj.ret === 1) {
                    $.msg($.name, "签到成功", obj.msg);
                    $.log("✅ 签到成功: " + obj.msg);
                } else {
                    $.msg($.name, "签到结果", obj.msg);
                    $.log("⚠️ 签到结果: " + obj.msg);
                }
            } catch (e) {
