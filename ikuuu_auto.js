...
function GetCookieOrCheckin() {
    if ($request && $request.headers) {
        ...
        if (cookie) {
            ...
            if (saved) {
                $.msg($.name + " 🍪", "Cookie 更新成功 🎉", "检测到 ikuuu.nl 访问，Cookie 已保存 🚀");
    ...
    // 如果是 request 也就是访问网页触发的，通常不需要立即签到，但原逻辑保留
    Checkin();
}
...
function Checkin() {
    ...
    if (!cookie) {
        $.msg($.name + " ⚠️", "未找到 Cookie", "请确保 MITM 包含 ikuuu.nl 并访问网页登录 🚫");
        $.log("❌ 未找到 Cookie");
        $.done();
        return;
    }
        ...
        if (error) {
            ...
            try {
                ...
                if (data && data.indexOf("404") !== -1) {
                    ...
                    $.msg($.name + " 🛑", "Cookie 已失效", "域名已变更，请重新访问 ikuuu.nl 获取 Cookie 🔄");
                    $.log("❌ Cookie 失效，返回了网页");
                } else {
                    $.log("❌ 解析失败: " + e);
                }
