...
const checkinUrl = "https://ikuuu.nl/user/checkin";
...
function GetCookieOrCheckin() {
    if ($request && $request.headers) {
        ...
        if (cookie) {
            ...
            if (saved) {
                $.msg($.name + " 🍪", "Cookie 更新成功 🎉", "现在的 Cookie 已经保存，可以自动签到啦 🚀");
...
function Checkin() {
    ...
    if (!cookie) {
        $.msg($.name + " ⚠️", "未找到 Cookie", "请先开启重写并访问网页(ikuuu.nl)登录获取 🚫");
        $.log("❌ 未找到 Cookie");
        $.done();
        return;
    }
        ...
        if (error) {
            ...
            try {
                ...
                if (data && data.indexOf("404") !== -1) {
                    ...
                    $.msg($.name + " 🛑", "Cookie 已失效", "请重新访问网页更新 Cookie 🔄");
                    $.log("❌ Cookie 失效，返回了网页");
                } else {
                    $.log("❌ 解析失败: " + e);
                }
